import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { buildProgram } from '../../index.js';
import { installFakeClient, runCli, silenceStdio, type FakeClient } from '../helpers.js';
import { resetClientFactory } from '../../lib/client.js';

let fake: FakeClient;

beforeEach(() => {
  process.env['FULKRUMA_KEY_ID'] = 'AKIA';
  process.env['FULKRUMA_SECRET'] = 'x';
  fake = installFakeClient();
});

afterEach(() => {
  resetClientFactory();
});

describe('integrations', () => {
  it('status calls integrations.status', async () => {
    fake.on('integrations.status', {});
    const s = silenceStdio();
    try {
      await runCli(buildProgram, ['integrations', 'status']);
    } finally {
      s.restore();
    }
    expect(fake.calls.some((c) => c.group === 'integrations' && c.method === 'status')).toBe(true);
  });
});
