...
router.get('/shipments/:id/label', requireAuth, async (req, res, next) => {
  try {
    const accountId = req.auth?.accountId;
    if (!accountId) return res.status(403).json(err('NO_ACCOUNT', 'token missing accountId', rid(req)));
    const requestedSize = String(req.query.size ?? 'thermal-100x150');
    if (!SHIPMENT_LABEL_SIZES.includes(requestedSize as ShipmentLabelSize)) {
      return res.status(400).json(err('VALIDATION', 'Unsupported label size', rid(req)));
    }
    let shipment = await prisma.shipment.findUnique({ where: { id: String(req.params.id) } });
    if (!shipment || shipment.accountId !== accountId) {
      return res.status(404).json(err('NOT_FOUND', 'Shipment not found', rid(req)));
    }
    ...
    if (!shipment.waybillId) {
      return res.status(409).json(err('NOT_READY', 'Label is available after the courier is booked and an AWB is issued', rid(req)));
    }
    const label = await generateShipmentLabel(shipment, { size: requestedSize as ShipmentLabelSize });
    return res.json(ok({ label }, rid(req)));
  } catch (e) { next(e); }
});
