...
function locationLabel(snapshot: Record<string, unknown>): string {
  return clean(
    pick(snapshot, 'city', 'administrative_division_level_2_name')
      || pick(snapshot, 'province', 'administrative_division_level_1_name')
      || pick(snapshot, 'postalCode', 'postal', 'postal_code')
...
function drawLabel(
  ...
  const courier = clean(shipment.courierCode).toUpperCase();
  ...
  doc.font('Helvetica-Bold').fontSize(compact ? 15 : 19).fillColor('#111111')
    .text(courier || 'COURIER', contentX, cursorY, { width: contentW * 0.45, lineBreak: false });
  doc.font('Helvetica-Bold').fontSize(compact ? 8 : 10)
    .text('FULKRUMA', contentX + (contentW * 0.5), cursorY + 1, { width: contentW * 0.5, align: 'right', lineBreak: false });
  doc.font('Helvetica').fontSize(compact ? 5 : 6)
    .text('forjio family · delivery powered by Biteship', contentX + (contentW * 0.42), cursorY + (compact ? 10 : 13), { width: contentW * 0.58, align: 'right', lineBreak: false });
  cursorY += compact ? 24 : 30;
  ...
  doc.font('Helvetica-Bold').fontSize(compact ? 8 : 10).fillColor('#111111')
    .text(waybill, contentX, cursorY, { width: contentW, align: 'center', lineBreak: false });
  cursorY += compact ? 13 : 17;

  const routeH = compact ? 19 : 24;
  doc.rect(contentX, cursorY, contentW, routeH).lineWidth(1).stroke();
  doc.font('Helvetica-Bold').fontSize(compact ? 7.5 : 9).text(
    `${locationLabel(origin)}  →  ${locationLabel(destination)}`,
    contentX + 4,
    cursorY + (compact ? 5 : 7),
    { width: contentW - 8, align: 'center', lineBreak: false, ellipsis: true },
  );
  cursorY += routeH + (compact ? 4 : 6);
  ...
  const addressH = compact ? 58 : 76;
  const senderLines = [senderName, ...formatAddress(origin)];
  if (options.showSenderPhone && senderPhone) senderLines.push(senderPhone);
  const recipientLines = [recipientName, ...formatAddress(destination)];
  if (options.showRecipientPhone && recipientPhone) recipientLines.push(recipientPhone);
  drawCell(doc, contentX, cursorY, half, addressH, 'Sender', senderLines, compact);
  drawCell(doc, contentX + half, cursorY, half, addressH, 'Recipient', recipientLines, compact);
