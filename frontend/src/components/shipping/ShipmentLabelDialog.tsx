...
interface Props {
  ...
  shipment: ShipmentLabelTarget | null;
  loadLabel: (shipmentId: string, options: ShipmentLabelOptions) => Promise<ShipmentLabelFile>;
}
...
export function ShipmentLabelDialog({ open, onOpenChange, shipment, loadLabel }: Props) {
  ...
  React.useEffect(() => {
    const saved = window.localStorage.getItem('forjio_shipment_label_size');
    if (formats.some((format) => format.value === saved)) setOptions((current) => ({ ...current, size: saved as ShipmentLabelSize }));
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    void loaderRef.current(shipment.id, options).then((file) => {
      if (cancelled) return;
      const url = shipmentLabelBlobUrl(file);
      if (previewUrlRef.current) URL.revokeObjectURL(previewUrlRef.current);
      previewUrlRef.current = url;
      setPreview({ url, file });
    }).catch((cause: unknown) => {
      if (!cancelled) { setPreview(null); setError(cause instanceof Error ? cause.message : 'Could not generate the shipping label'); }
  ...
  function printLabel() {
    if (!preview) return;
    try { const frame = iframeRef.current?.contentWindow; if (!frame) throw new Error(); frame.focus(); frame.print(); }
    catch { if (!window.open(preview.url, '_blank', 'noopener,noreferrer')) toast.error('Your browser blocked the label tab. Allow pop-ups and try again.'); }
  }
  function downloadLabel() {
    if (!preview) return;
    const link = document.createElement('a'); link.href = preview.url; link.download = preview.file.fileName; document.body.appendChild(link); link.click(); link.remove();
  }
...
function Toggle({ id, label, checked, disabled, onChange }: { id: string; label: string; checked: boolean; disabled?: boolean; onChange: (checked: boolean) => void }) {
  return <label htmlFor={id} className={`flex items-center gap-2 text-xs ${disabled ? 'text-muted-foreground/60' : 'text-muted-foreground'}`}><Checkbox id={id} checked={checked} disabled={disabled} onCheckedChange={(value) => onChange(value === true)} />{label}</label>;
}
