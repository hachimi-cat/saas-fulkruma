 ...
 * sheet's plan transport, which fulkruma doesn't mount yet.
 */
...
export const ASSISTANT_ACTIVITY_EVENT = 'fulkruma:assistant-activity';
...
export function useCatentioStatus(): { enabled: boolean; loading: boolean } {
  ...
  useEffect(() => {
    let cancelled = false;
    api<{ enabled?: boolean }>('/catentio/status')
      .then((data) => {
...
export function useAssistantSettings(enabled: boolean): {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api<{ autoApply?: boolean }>('/catentio/settings')
      .then((data) => {
        if (cancelled) return;
...
export function useCatentioCredits(enabled: boolean): {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    api<CatentioCredits>('/catentio/credits')
      .then((data) => {
...
export async function startCreditsTopup(
  ...
  return api<{ checkoutUrl: string | null; noop: boolean; credits: number }>(
    '/catentio/credits/topup',
